...
FROM 
	authors
WHERE 
	au_lname = 'Ringer' AND au_fname = 'Anne'
;
...
SELECT pub_id, type, title, COUNT(*)
FROM titles
...
SELECT pub_id, type, title, COUNT(*), AVG(price)
FROM titles
GROUP BY pub_id, type
HAVING AVG(price > 12)
...
SELECT city, COUNT(CONCAT(au_fname, ' ', au_lname))
FROM authors
WHERE state = 'CA' AND city LIKE '%o%'
GROUP BY city
HAVING COUNT(CONCAT(au_fname, ' ', au_lname))
ORDER BY COUNT(CONCAT(au_fname, ' ', au_lname));